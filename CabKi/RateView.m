...
- (void)layoutSubviews {
    ...
    float desiredImageWidth = (self.frame.size.width-30 - (self.leftMargin*2) - (self.midMargin*self.imageViews.count)) / self.imageViews.count;
   
    
    float imageWidth = MAX(self.minImageSize.width, desiredImageWidth);
   
    
    float imageHeight = MAX(self.minImageSize.height, self.frame.size.height-30);
    ...
    for (int i = 0; i < self.imageViews.count; ++i) {
        ...
        UIImageView *imageView = [self.imageViews objectAtIndex:i];
        CGRect imageFrame ;
      imageFrame = CGRectMake(self.leftMargin+imageWidth + i*(self.midMargin+imageWidth), 15, imageWidth, imageHeight);
